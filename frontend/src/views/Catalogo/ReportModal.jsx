...
import api from '../../services/api'; // Importamos tu cliente de Axios

export default function ReportModal({ isOpen, onClose, initialScope = 'global', initialSubjectId = null }) {
  const [scope, setScope] = useState(initialScope);
  const [period, setPeriod] = useState('mensual'); // Por defecto a mensual
  const [isGenerating, setIsGenerating] = useState(false);

  // Sincroniza el scope inicial cuando se abre el modal
  useEffect(() => {
    if (isOpen) {
  ...
  // Lógica de conexión con los endpoints GET del Resumen
  const handleExport = async (formato) => {
    setIsGenerating(true);
    
    // Mapeo del período del frontend al query esperado por tu backend
    let queryPeriodo = period === 'mensual' ? 'mes' : period === 'semanal' ? 'semana' : 'dia';
    let endpoint = '';

    // Construcción de la URL dinámica
    if (scope === 'global') {
      endpoint = `/reportes/general?periodo=${queryPeriodo}`;
    } else if (scope === 'usuario') {
      endpoint = `/reportes/usuario/${initialSubjectId || 1}?periodo=${queryPeriodo}`;
    } else {
      // Ruta teórica para insumo específico basada en tu estructura
      endpoint = `/reportes/insumo/${initialSubjectId || 1}?periodo=${queryPeriodo}`;
    }

    try {
      const response = await api.get(endpoint);
      console.log(`[EXITO] Datos del reporte ${formato.toUpperCase()}:`, response.data);
      
      // Aquí iría la lógica para procesar el Blob y forzar la descarga del PDF/Excel
      alert(` Datos obtenidos correctamente del backend.\nEndpoint: ${endpoint}\nRevisa la consola para ver la estructura.`);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[60] flex items-center justify-center p-4 transition-opacity" onClick={onClose}>
      <div className="bg-app rounded-2xl w-full max-w-[500px] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-fade-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 border-t border-border bg-card flex items-center justify-end gap-2">
          <button onClick={onClose} disabled={isGenerating} className="px-4 py-2 text-sm font-heading font-semibold text-text-secondary hover:bg-border rounded-lg mr-auto">
            Cancelar
          </button>
          <button 
            onClick={() => handleExport('pdf')} disabled={isGenerating}
            className="flex items-center gap-2 px-4 py-2 bg-inputBg border border-border text-text-primary hover:border-accent rounded-lg font-heading font-semibold text-sm transition-colors disabled:opacity-50"
          >
            PDF
          </button>
          <button 
            onClick={() => handleExport('excel')} disabled={isGenerating}
            className="flex items-center gap-2 px-4 py-2 bg-[#10B981] text-white hover:bg-[#059669] rounded-lg font-heading font-semibold text-sm transition-colors disabled:opacity-50"
          >
            Excel
          </button>
        </div>
